<img align="right" src="https://raw.githubusercontent.com/hxfuxyy/InfinityX-XiaomiPad5/main/nabu.png" width="425" alt="InfinityX ROM на Xiaomi Pad 5">

# Установка InfinityX на Xiaomi Pad 5

[![Telegram](https://img.shields.io/badge/Chat-Telegram-brightgreen.svg?logo=telegram&style=flat-square)](https://t.me/InfinityXnabu)
[![Donate](https://img.shields.io/badge/Donate-Support%20Us-orange.svg?style=flat-square)](https://linktr.ee/zxcwsurx)
[![Latest Release](https://img.shields.io/github/v/release/hxfuxyy/infota?label=Latest%20Release&style=flat-square)](https://github.com/hxfuxyy/infota/releases/latest)
[![Total Downloads](https://img.shields.io/github/downloads/hxfuxyy/infota/total?label=Total%20Downloads&style=flat-square)](https://github.com/hxfuxyy/infota/releases)

## Язык
[`English`](/README.md) | [`Українська`](/guide/Ukrainian/README-uk.md) | **`Русский`**

## ⚠️ Предупреждение

> [!NOTE]
> - Мы не несём ответственности за кирпичи, повреждённые разделы восстановления, потерянные данные, недовольных котов, экзистенциальный кризис или то, что вы проспали пары, потому что ночью прошивали ROM.
> - Это неофициальная сборка, поддерживаемая сообществом. Выполняйте каждый шаг внимательно и делайте это на свой страх и риск.

> [!WARNING]
> **Если вы не уверены насчёт разблокировки загрузчика, прошивки кастомного рекавери или выполнения ADB-команд — сначала прочитайте руководство целиком. Процесс прямолинейный, но пропуск шагов сломает устройство.**

> [!CAUTION]
> **Всегда делайте резервную копию важных данных перед прошивкой. Чистая установка требует сброса до заводских настроек и полного удаления всех данных внутреннего хранилища.**

## Начало работы

- [**`📥 Установка`**](installation-ru.md)

## Дополнительно

- [**`❗ Решение проблем`**](troubleshooting-ru.md)
- [**`📱 Получение root-прав`**](root-ru.md)
- [**`🔓 Разблокировка загрузчика`**](https://github.com/erdilS/Port-Windows-11-Xiaomi-Pad-5/blob/main/guide/English/unlock-bootloader-en.md)
- [**`💾 Руководство по TWRP`**](https://github.com/ArKT-7/twrp_device_xiaomi_nabu/releases/tag/mod-win)

## Мейнтейнеры

[<img alt="hxfuxyy" src="https://images.weserv.nl/?url=https://avatars.githubusercontent.com/u/hxfuxyy?v=4&w=45&fit=cover&mask=circle&maxage=7d" />](https://github.com/hxfuxyy)
[<img alt="tvorogo" src="https://images.weserv.nl/?url=https://avatars.githubusercontent.com/u/tvorogo?v=4&w=45&fit=cover&mask=circle&maxage=7d" />](https://github.com/tvorogo)

## Благодарности

- [**ProjectInfinity-X**](https://github.com/ProjectInfinity-X) — базовый ROM
- [**ArKT-7**](https://github.com/ArKT-7) — AutoInstaller-Forge, TWRP & много фиксов
- [**dev-harsh1998**](https://github.com/dev-harsh1998) — девайс три
- [**KernelSU-Next team**](https://github.com/KernelSU-Next/KernelSU-Next) — рут-решение KernelSU-Next
- [**erdilS и контрибьюторы**](https://github.com/erdilS/Port-Windows-11-Xiaomi-Pad-5) — структура документации для nabu
- Всем в [t.me/InfinityXnabu](https://t.me/InfinityXnabu), кто тестирует, репортит баги и поддерживает проект живым ❤️
