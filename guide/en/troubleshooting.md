...
- On Windows: install or reinstall [Google USB Driver](https://developer.android.com/studio/run/win-usb) or use a universal driver like [Universal ADB Driver](https://adb.clockworkmod.com/)
...
- Then re-flash the ROM via ADB sideload or Auto Installer
...
- First boot can take **5–10 minutes** — wait it out before assuming something is wrong
- If it exceeds 15 minutes, reboot into recovery and perform a Factory Reset, then re-flash

</details>

<details>
<summary>ADB sideload fails or shows error</summary>
...
- Use the `-d` flag to target the device directly: `adb -d sideload InfinityX.zip`
- Try running the command with administrator privileges (Windows) or `sudo` (Linux/macOS)
...
- On Linux/macOS: ensure the script has execute permissions — `chmod +x install_..._linux.sh`
